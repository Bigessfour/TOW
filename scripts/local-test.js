...
function runCommand(command, description, optional = false) {
  try {
    ...
    if (optional) {
      console.log(
        `${colors.yellow}⚠️  ${description} skipped (optional)${colors.reset}`
      );
      return { success: true, output: '' };
    } else {
...
async function main() {
  ...
  totalTests++;
  if (
    runCommand('npm list --depth=0', 'NPM dependencies check', true).success
  ) {
    passedTests++;
  }
  ...
  if (checkFileExists('.htmlhintrc', 'HTMLHint configuration')) {
    if (
      runCommand('npx htmlhint index.html', 'HTML structure validation', true)
        .success
    ) {
      passedTests++;
    }
  ...
  totalTests++;
  if (
    runCommand('npm run test:html', 'HTML validation with html-validate', true)
      .success
  ) {
    passedTests++;
  }
  ...
  if (fs.existsSync('assets/js') && fs.readdirSync('assets/js').length > 0) {
    ...
    if (autoFix) {
      console.log(
        `${colors.blue}🔧 Auto-fixing JavaScript issues...${colors.reset}`
      );
      runCommand(
  ...
  if (
    runCommand(
      'npx markdownlint README.md docs/**/*.md _pages/**/*.md _posts/**/*.md',
      'Markdown linting',
      true
    ).success
  ) {
  ...
  totalTests++;
  if (
    runCommand('bundle exec jekyll doctor', 'Jekyll configuration check', true)
      .success
  ) {
    passedTests++;
  }
  ...
  if (!formatOnly) {
    ...
    if (checkFileExists('lighthouserc.json', 'Lighthouse configuration')) {
      console.log(
        `${colors.blue}ℹ️  Performance testing requires a running server${colors.reset}`
      );
      console.log(
        `${colors.blue}ℹ️  Run 'npm run serve:jekyll' in another terminal and test manually${colors.reset}`
      );
    }
  }
  ...
  if (autoFix && !formatOnly) {
    ...
    runCommand(
      ...
      'Prettier JavaScript fix',
      true
    );

    // Fix CSS
    runCommand(
      'npx prettier --write "assets/css/**/*.css"',
      'Prettier CSS fix',
      true
    );
    ...
    // Fix Markdown
    runCommand(
      'npx markdownlint --fix README.md docs/**/*.md _pages/**/*.md _posts/**/*.md',
      'Markdownlint fix',
      true
    );
...
if (args.includes('--help') || args.includes('-h')) {
  ...
  console.log(
    '  node local-test.js --format-only      # Quick formatting check'
  );
  console.log(
    '  node local-test.js --auto-fix         # Fix formatting automatically'
  );
  process.exit(0);
}
